...
for k = numC
  ...
  Nc = [];
  Vc = [];
  for i = 1:length(tmp.clustFilt) % control.clustOrder
  ...
  % logL = -Nc.*nansum(.5*log(Vc+V),1);
  logL = -Nc.*nansum(.5*log(Vc),1); % there is no cluster with only 1 element
